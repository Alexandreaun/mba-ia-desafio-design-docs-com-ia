...
- Hipótese (não sustentada por `TRANSCRICAO.md`, que não registra decisão sobre uma etapa formal de validação ponta a ponta com cliente real antes do go-live): como validação funcional complementar, o time pode optar por simular o fluxo completo de um dos três clientes citados (cadastro de webhook, mudança de status de um pedido de teste, recebimento do evento, consulta de histórico de entregas) antes da liberação a qualquer cliente em produção; alternativas equivalentes seriam validação apenas com dados sintéticos em ambiente de homologação, ou liberação gradual (feature flag por cliente) sem simulação prévia dedicada.
